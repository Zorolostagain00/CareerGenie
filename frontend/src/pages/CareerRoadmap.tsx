    ...
    if (!state.results || !state.results.roadmaps) {
        return (
            <div className="min-h-screen pt-32 flex flex-col items-center">
                <p className="text-slate-600 mb-6">No roadmaps generated yet.</p>
    ...
    return (
        <div className="min-h-screen pt-24 pb-24 max-w-4xl mx-auto px-4">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-16">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-teal-50 border border-teal-100 text-teal-800 text-sm font-medium mb-6">
                    <Map className="w-4 h-4" />
                    <span>Execution Plan</span>
                </div>
                <h1 className="text-3xl md:text-5xl font-bold text-slate-900 mb-4">Your Career Roadmaps</h1>
                ...
                {top_3_roles.map((role: any, idx: number) => {
                    ...
                    return (
                        <motion.div
                            ...
                            className="bg-white/80 border border-slate-200 rounded-2xl overflow-hidden shadow-sm"
                        >
                            <div className="bg-slate-50/80 px-8 py-5 border-b border-slate-200 flex justify-between items-center">
                                <h2 className="text-2xl font-bold text-slate-900 flex items-center gap-3">
                                    <span className="text-indigo-600">#{idx + 1}</span> {role.title}
                                </h2>
                                <span className="text-xs font-semibold px-2 py-1 rounded bg-white border border-slate-300 text-slate-700">
                                    {role.cluster}
                                </span>
                            </div>

                            <div className="p-8 prose prose-indigo max-w-none 
                prose-h3:text-lg prose-h3:font-semibold prose-h3:text-slate-900 prose-h3:mt-8 prose-h3:mb-3 first:prose-h3:mt-0
                prose-p:text-slate-600 prose-p:leading-relaxed prose-p:mb-6
                prose-ul:text-slate-600 prose-li:my-1 prose-ul:mb-6"
                            >
            ...
            <div className="mt-20 pt-10 border-t border-slate-200 flex justify-center">
                <button
                    ...
                    className="flex items-center gap-2 px-6 py-3 border border-slate-300 hover:bg-slate-100 text-slate-700 rounded-lg font-medium transition-colors"
                >
